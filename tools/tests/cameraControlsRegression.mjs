...
try {
  ...
  for (let index = 1; index < transferFrames.length; index += 1) {
    ...
    stepDistancesKm.push(cameraDistance(previous, current));
  }
  ...
  assert.ok(Math.max(...stepDistancesKm) < travelKm * 0.04, 'transfer contains a discontinuous camera step');
    ...
    `${JSON.stringify(
      {
        ...
        travelKm,
        maximumStepKm: Math.max(...stepDistancesKm),
