...
import { ingestSkyTextures } from './skyTextures.mjs';
...
export async function ingestAssets(options) {
  ...
  const skyAssets = (options.skyAssets ?? SKY_TEXTURE_ASSETS).filter(
    (asset) => selectedIds === null || selectedIds.has(asset.id),
  );
  const discovered = (await discoverAssets(modelsRoot))
    .filter((asset) => selectedIds === null || selectedIds.has(asset.id));
  if (discovered.length === 0 && skyAssets.length === 0) {
    throw new Error(`No assets discovered under ${modelsRoot}`);
  }
  ...
  try {
    ...
    manifestAssets.push(
      ...(await ingestSkyTextures({
        skyRoot: options.skyRoot ?? join(modelsRoot, '..', 'textures-src'),
        stagingRoot,
        assets: skyAssets,
        ...(options.encodeTexture !== undefined ? { encodeTexture: options.encodeTexture } : {}),
        ...(options.ktxExecutable !== undefined ? { ktxExecutable: options.ktxExecutable } : {}),
      })),
    );
