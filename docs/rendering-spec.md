...
2. **Tiling detail maps (the workhorse):** seamless 1k detail normal + albedo-variation pairs per surface class (rock, ice, regolith, gas banding), blended with a cubic fade from exactly zero at 5 body-radii to fully active at 1.2 radii. Runtime asset-manifest schema 2 stores each pair, `tilesPerEquator`, and a deterministic uint32 seed. The shader samples a macro tile and a phase-shifted micro tile at 8× frequency; centered albedo variation has strength 0.12 and normal XY weights are 0.08/0.03. The exact-zero branch leaves far output byte-identical.
3. **Procedural shader noise:** a shared seeded two-octave, seam-free direction-space modulation fades in from 1.5 to 1.2 radii. It perturbs tangent-space normal at strength 0.12 and roughness at 0.16, reusing the hero normal map's tangent frame where available.
