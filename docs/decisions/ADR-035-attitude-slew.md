...
3. **Thrust follows the attitude actually held**, not the target — otherwise the
   ship would accelerate in a direction it is visibly not pointing.
...
4. **The converged branch copies the target verbatim.** When
   ...
   - Every hold reference in the game rotates far below 15°/s (LEO orbital rate
     ≈ 1.1e-3 rad/s, four hundred times slower), so a converged hold takes this
     branch at *every* stage and evaluates to `q_target(t)` — not an
   ...
   - It makes `maxSlewRadPerSimS → ∞` an exact reproduction of the old law,
     which is how the LEO regression obtains a pre-change baseline to diff
     against without committing a new fixture.
