...
Measured with `measureBundleSizes('dist')` on production builds of the base
commit and this branch, the same function the gate uses.

| Metric     |    Before |     After |        Delta |
| ---------- | --------: | --------: | -----------: |
| Entry gzip | 129,865 B | 133,543 B | **+3,678 B** |
| Total gzip | 559,645 B | 563,689 B | **+4,044 B** |

Entry is far inside its 285,000 B ceiling. **Total gzip is not comfortable:** the
golden ceiling is 570,000 B, so after this PR the headroom is **6,311 B (1.1%)**,
