...
- Version migrations are explicit and each is covered by a committed fixture (`tests/fixtures/save-v1.json`, `tests/fixtures/save-v2.json`); a migrated document adopts the running vessel. Rails bodies are never serialized because their positions and velocities are deterministically derived from `simTimeSec`.
