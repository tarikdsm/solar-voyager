...
When detail blend is exactly zero, the shader skips every added texture sample and procedural operation. The vertex shader initializes its procedural varying to the neutral value and enters the wave evaluation only when the procedural blend is nonzero. No base color, normal, roughness, emissive, alpha, or lighting value is rewritten. This is the contract behind the byte-identical far-view regression.

When enabled, the shader samples the seamless pair at `uv * tilesPerEquator` and at approximately eight times that frequency with a deterministic phase offset and a rotated UV basis. Centered sRGB albedo is decoded around its linear mid-gray identity; the two normal samples are combined with the authored macro normal instead of replacing it. At the closest range, seeded two-octave periodic C1 waves over normalized object-space direction add low-amplitude roughness and normal breakup without a longitude seam. The periodic triangle waves pass through a cubic fade so both value and derivative meet at each `fract` boundary. They are evaluated per hero-mesh vertex and smoothly interpolated for fragment normal/roughness modulation; the KTX2 octaves remain the per-fragment high-frequency layer. Interpolated lattice value noise was rejected after exact-head benchmarking because its per-fragment cost violated the conservative hardware proxy budget. All loop counts and branches are compile-time bounded.
