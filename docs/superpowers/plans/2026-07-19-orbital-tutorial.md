...
- Add failing tests in `src/game/settings.test.ts`, `src/game/saveLoad.test.ts`, and `src/game/sessionController.test.ts` for:
  ...
  - invalid present v2 never falls back to v1, while valid v1 migration is written immediately and fails closed if persistence fails;
...
- Implement `src/game/tutorialController.ts` with no DOM, Preact, simulation mutation, or timers. Route functional tutorial updates through the session controller as the sole profile owner.
- Make every resumed step repeatable or satisfiable from current real state; test reload/resume directly at `thrust-off`, `map-return`, and `burn-log`.
...
- Extend camera/input tests first for Shift+Arrow/Shift+Page Up/Page Down controls, gameplay suppression for new Shift-modified keydowns regardless of rebound code, Shift-modified keyup still releasing an already-held flight axis, editable-target suppression, and interaction callbacks.
...
- Wire camera, map, 10 Hz real snapshot/view state, burn log, performance, warning, and save callbacks to the controller. Snapshot/view baselines, not command calls, are the oracle for target, attitude, thrust, and warp.
...
- Cover full guided completion, reload persistence/no overlay, skip/resume/reset, keyboard-only camera/flight controls, compact 360x480 bounds/non-obstruction, computed reduced-motion styles, focus movement, conditional hardware warning, false terminal observer, and zero console/page errors. Page evaluation may read diagnostics but never call controller or `Commands`.
...
- Record `npm run bench` baseline before frame-loop changes and after implementation. Run format, lint, typecheck, all Vitest tests, build, performance gates, application smoke, and all affected browser regressions.
